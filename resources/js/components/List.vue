            ...
            <loading-view :loading="loading">
                ...
                <div class="bg-20 rounded-b">
                    <nav class="flex justify-between items-center">
                        <router-link :to="`${resourceName}?page=${query.page - 1}`" v-bind:disabled="!hasPreviousPage" v-bind:class="{'opacity-50': !hasPreviousPage}" tag="button" rel="prev" class="btn btn-link py-3 px-4 text-80">Previous</router-link>
                        <span class="text-sm text-80 px-4">{{query.page * query.pageSize + 1}}-{{Math.min((query.page + 1) * query.pageSize, count)}} of {{count}}</span>
                        <router-link :to="`${resourceName}?page=${query.page + 1}`" v-bind:disabled="!hasNextPage" v-bind:class="{'opacity-50': !hasNextPage}" tag="button" rel="next" class="btn btn-link py-3 px-4 text-80">Next</router-link>
                    </nav>
                </div>
            </loading-view>
        </card>
