<template>
  <div>
    <heading class="mb-6">Klicktipp Tags Periods</heading>
    <div class="flex" style="">
      <div class="w-full flex items-center">
        <!---->
        <div class="flex-no-shrink ml-auto mb-6">
          <router-link
              to="/novashopengine/settings/newsletter-provider/klicktipp/tags-periods/create"
    ...
    <div class="card" v-if="!tagsPeriods.length">
    ...
    <card class="mb-4" v-if="tagsPeriods.length">
      <div class="border-b border-40">
        <div class="py-6 px-8">
          <table class="table w-full">
            ...
            <tr v-for="tagPeriod in tagsPeriods">
              <td>{{tagPeriod.tag }}</td>
              <td>{{tagPeriod.from }}</td>
              <td>{{tagPeriod.to }}</td>
              <td class="td-fit text-right pr-6 align-middle">
                <div class="inline-flex items-center">
                    <span class="inline-flex">
                      <router-link
                      :to="'/novashopengine/settings/newsletter-provider/klicktipp/tags-periods/' + tagPeriod.tag"
                ...
                <span class="inline-flex">
                      <router-link
                  :to="'/novashopengine/settings/newsletter-provider/klicktipp/tags-periods/' + tagPeriod.tag + '/edit'"
                        class="inline-flex cursor-pointer text-70 hover:text-primary mr-3 has-tooltip"
                        dusk="6-edit-button"
                        data-original-title="null"
                ...
                <button
                    ...
                    data-original-title="null"
                @click="activeModalFor(tagPeriod.tag)"
                >
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="20"
...
@submit.prevent="deleteTag()"
    >
    <div class="p-8">
    <h2 class="mb-6 text-90 font-normal text-xl">Delete Klicktipp</h2>
...
<script>
  ...
  `/nova-vendor/novashopengine/settings/newsletter-provider/klicktipp/tags-periods/${this.tagToDelete}/delete`,
{}
  );
  if (data.success) {
  this.$toasted.show(`${data.message}`, { type: "warning" });
  this.isModalActive = false;
  this.tagsPeriods = data.data;
} else {
  this.$toasted.show(`Error: ${data.message}`, { type: "error" });
}
